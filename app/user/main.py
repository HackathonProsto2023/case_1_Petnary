...
def get_hard_rate() -> Response:
    ...
    data = request.json or {}
    if 'correct_answer' not in data or 'users_answer' not in data:
        return bad_request('must include correct_answer and users_answer field')
    user_id = get_jwt_identity()
    user = Users.query.filter_by(id=user_id).first_or_404()
    rate = 0
    for i in data:
        if i.correct_answer == i.users_answer:
            rate += 1
    user.hard_rate = rate
    db.session.commit()
